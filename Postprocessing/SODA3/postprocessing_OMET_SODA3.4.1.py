...
Function        : Postprocessing meridional energy transport from Cartesius (SODA3.4.1)
Author          : Yang Liu
Date            : 2018.03.05
...
Description     : The code aims to postprocess the output from the Cartesius
                  ...
                  transport based on oceanic reanalysis dataset SODA3 from University of Maryland & TAMU.
...
variables       : Meridional Energy Transport               E
                  Land-Sea Mask                             mask
Caveat!!        : The full dataset is from 1980 to 2015. The data coverage is the entire globe.
"""
...
################################   Input zone  ######################################
# specify data path
datapath = '/home/yang/workbench/Core_Database_AMET_OMET_reanalysis/SODA3/postprocessing'
# specify output path for the netCDF4 file
output_path = '/home/yang/workbench/Core_Database_AMET_OMET_reanalysis/SODA3/postprocessing'
Lat_num = 60 # gphiv = 789
# mask path
mask_path = '/home/yang/workbench/Core_Database_AMET_OMET_reanalysis/SODA3'
####################################################################################
print '*******************************************************************'
...
level = 50
# zonal integral
dataset = Dataset(datapath + os.sep + 'OMET_SODA3_model_5daily_1980_2015_E_zonal_int.nc')
# spacial distribution
dataset_point = Dataset(datapath + os.sep + 'OMET_SODA3_model_5daily_1980_2015_E_point.nc')
# load land-sea mask INFO
dataset_mask = Dataset(mask_path + os.sep + 'topog.nc')
...
fig1 = plt.figure()
plt.plot(index,T_series_E[789,:]/1000,'b-',label='SODA3') # lat = 60N
plt.title('Oceanic Meridional Energy Transport time series at %d N (1980-2015)' % (Lat_num))
#plt.legend()
...
plt.show()
fig1.savefig(output_path + os.sep + 'OMET_SODA3_%dN_total_time_series_1980_2015.jpg' % (Lat_num), dpi = 500)

print '*******************************************************************'
print '*********************** time series shades ************************'
print '*******************************************************************'

x , y = np.meshgrid(index,latitude_aux[569:])

fig2 = plt.figure()
#plt.contour(x,y,T_series_E/1000, linewidth=0.05, colors='k')
# cmap 'jet' 'RdYlBu' 'coolwarm'
plt.contourf(x,y,T_series_E[569:,:]/1000,cmap='coolwarm') # from 20N-90N
plt.title('Oceanic Meridional Energy Transport time series(1980-2015)' )
...
plt.show()
fig2.savefig(output_path + os.sep + 'OMET_SODA3_time_series_1980_2015_shades.jpg', dpi = 500)
...
plt.plot(latitude_aux,np.mean(series_E,0)/1000,'b-',label='SODA3')
plt.title('Oceanic Meridional Energy Transport (1980-2015)' )
#plt.legend()
plt.xlabel("Latitudes")
#plt.xticks()
plt.ylabel("Meridional Energy Transport (PW)")
plt.show()
fig3.savefig(output_path + os.sep + 'OMET_SODA3_mean_1980_2015.jpg', dpi = 500)
...
contour_level = np.arange(-20,60,2)
...
plt.show()
fig11.savefig(output_path + os.sep + "OMET_SODA3_StreamFunction_Globe.png",dpi=500)

# Atlantic
fig12 = plt.figure()
X , Y = np.meshgrid(latitude_aux[361:],level) # from 30S to 90N
contour_level = np.arange(-30,30,3)
plt.contour(X,Y,np.mean(Psi_atl_mean[:,:,361:],0),linewidth= 0.2)
cs = plt.contourf(X,Y,np.mean(Psi_atl_mean[:,:,361:],0),contour_level,linewidth= 0.2,cmap='RdYlGn')
...
plt.gca().invert_yaxis()
plt.show()
fig12.savefig(output_path + os.sep + "OMET_SODA3_StreamFunction_Atlantic.png",dpi=500)
...
fig4 = plt.figure()
plt.axes(polar = True)
plt.plot(angle_series,T_series_E[789,:]/1000,'b--',label='SODA3')
plt.title('Oceanic Meridional Energy Transport at %d N (1980-2015)' % (Lat_num), y=1.07)
#plt.legend()
...
#plt.ylabel("Meridional Energy Transport (PW)")
plt.show()
fig4.savefig(output_path + os.sep + 'OMET_SODA3_%dN_total_windrose_1980_2015.jpg' % (Lat_num), dpi = 500)

# wind rose of time series after removing seasonal cycles

fig5 = plt.figure()
plt.axes(polar = True)
plt.plot(angle_series,T_series_E_white[789,:]/1000,'b--',label='SODA3')
plt.title('Oceanic Meridional Energy Transport anomaly at %d N (1980-2015)' % (Lat_num), y=1.07)
#plt.legend()
#fig10.set_size_inches(14, 4)
#plt.xlabel("Time")
plt.xticks(angle[:-1], month_str)
...
#plt.ylabel("Meridional Energy Transport (PW)")
plt.show()
fig5.savefig(output_path + os.sep + 'OMET_SODA3_%dN_total_windrose_white_1980_2015.jpg' % (Lat_num), dpi = 500)
