...
Function        : Regress climate patterns on oceanic meridional energy transport in Atlantic only (ORAS4,GLORYS2V3,SODA3)
Author          : Yang Liu
Date            : 2018.03.06
Last Update     : 2018.07.07
Description     : The code aims to regress non-climatological fields on the oceanic
                  meridional energy transport ONLY IN THE ATLANTIC OCEAN, which are
                  calculated from different oceanic reanalysis datasets. In this case,
                  this includes GLORYS2V3 from Mercator Ocean, ORAS4 from ECMWF,
...
output_path = '/home/yang/NLeSC/Computation_Modeling/BlueAction/OMET/Comparison/regress/Atlantic/'
...
lat_interest = {}
lat_interest_list = [20,30,40,50,60,70]

lat_interest['ORAS4'] = [lat_ORAS4_20,lat_ORAS4_30,lat_ORAS4_40,lat_ORAS4_50,lat_ORAS4_60,lat_ORAS4_70]
lat_interest['GLORYS2V3'] = [lat_GLORYS2V3_20,lat_GLORYS2V3_30,lat_GLORYS2V3_40,lat_GLORYS2V3_50,lat_GLORYS2V3_60,lat_GLORYS2V3_70]
lat_interest['SODA3'] = [lat_SODA3_20,lat_SODA3_30,lat_SODA3_40,lat_SODA3_50,lat_SODA3_60,lat_SODA3_70]
print '*******************************************************************'
...
OMET_ORAS4_point = dataset_ORAS4.variables['E'][21:,:,:,:]/1000 # start from 1979
OMET_GLORYS2V3_point = dataset_GLORYS2V3.variables['E'][:]/1000 # start from 1993
OMET_SODA3_point = dataset_SODA3.variables['E'][:]/1000 # start from 1980
...
OMET_ORAS4_series = OMET_ORAS4.reshape(len(year_ORAS4)*len(month_ind),len(lat_ORAS4[:,0]))
OMET_GLORYS2V3_series = OMET_GLORYS2V3.reshape(len(year_GLORYS2V3)*len(month_ind),len(lat_GLORYS2V3[:,0]))
OMET_SODA3_series = OMET_SODA3.reshape(len(year_SODA3)*len(month_ind),len(lat_SODA3[:,0]))
...
OMET_ORAS4_white_series = OMET_ORAS4_white.reshape(len(year_ORAS4)*len(month_ind),len(lat_ORAS4[:,0]))
OMET_GLORYS2V3_white_series = OMET_GLORYS2V3_white.reshape(len(year_GLORYS2V3)*len(month_ind),len(lat_GLORYS2V3[:,0]))
OMET_SODA3_white_series = OMET_SODA3_white.reshape(len(year_SODA3)*len(month_ind),len(lat_SODA3[:,0]))
...
print '***************************  Detrend  *****************************'
print '*******************************************************************'
time_ERAI = np.arange(len(year_ERAI)*12)
####################################################
...
for i in np.arange(len(lat_ORAS4)):
...
for i in np.arange(len(lat_GLORYS2V3)):
...
for i in np.arange(len(lat_SODA3)):
...
OMET_ORAS4_series_running_mean = np.zeros((len(OMET_ORAS4_series)-window+1,len(lat_ORAS4[:,0])),dtype=float)
OMET_GLORYS2V3_series_running_mean = np.zeros((len(OMET_GLORYS2V3_series)-window+1,len(lat_GLORYS2V3[:,0])),dtype=float)
OMET_SODA3_series_running_mean = np.zeros((len(OMET_SODA3_series)-window+1,len(lat_SODA3[:,0])),dtype=float)

OMET_ORAS4_white_series_running_mean = np.zeros((len(OMET_ORAS4_white_series)-window+1,len(lat_ORAS4[:,0])),dtype=float)
OMET_GLORYS2V3_white_series_running_mean = np.zeros((len(OMET_GLORYS2V3_white_series)-window+1,len(lat_GLORYS2V3[:,0])),dtype=float)
OMET_SODA3_white_series_running_mean = np.zeros((len(OMET_SODA3_white_series)-window+1,len(lat_SODA3[:,0])),dtype=float)

SLP_ERAI_white_series_running_mean = np.zeros((len(time_series)-window+1,len(latitude_ERAI_fields),len(longitude_ERAI_fields)),dtype=float)
SST_ERAI_white_series_running_mean = np.zeros((len(time_series)-window+1,len(latitude_ERAI_fields),len(longitude_ERAI_fields)),dtype=float)
TS_ERAI_white_series_running_mean = np.zeros((len(time_series)-window+1,len(latitude_ERAI_fields),len(longitude_ERAI_fields)),dtype=float)

for i in np.arange(len(OMET_ORAS4_white_series)-window+1):
    for j in np.arange(len(lat_ORAS4[:,0])):
...
for i in np.arange(len(OMET_GLORYS2V3_white_series)-window+1):
    for j in np.arange(len(lat_GLORYS2V3[:,0])):
...
for i in np.arange(len(OMET_SODA3_white_series)-window+1):
    for j in np.arange(len(lat_SODA3[:,0])):
...
for i in np.arange(len(time_series)-window+1):
...
for c in np.arange(len(lat_interest_list)):
    ...
    for i in np.arange(len(latitude_ERAI_fields)):
        for j in np.arange(len(longitude_ERAI_fields)):
            ...
            slope_ERAI_fields[i,j],_,r_value_ERAI_fields[i,j],p_value_ERAI_fields[i,j],_ = stats.linregress(OMET_ORAS4_white_series[:,lat_interest['ORAS4'][c]],SLP_ERAI_white_series[:-24,i,j])
    ...
    fig1.suptitle('Regression of SLP Anomaly from ERA-Interim on OMET Anomaly (Atlantic) of ORAS4 across %d N' % (lat_interest_list[c]) ,fontsize = 7,y=0.93)
    ...
    # show and save plot
    plt.show()
    fig1.savefig(output_path + os.sep + 'SLP' + os.sep + 'OMET_ORAS4_fields_ERAI' + os.sep + "Regression_OMET_ORAS4_atl_%dN_SLP_ERAI_white_correlation_coef.jpeg" % (lat_interest_list[c]),dpi=300)
    plt.close(fig1)
    # @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@      GLORYS2V3      @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ #
    for i in np.arange(len(latitude_ERAI_fields)):
        for j in np.arange(len(longitude_ERAI_fields)):
            # return value: slope, intercept, r_value, p_value, stderr
            slope_ERAI_fields[i,j],_,r_value_ERAI_fields[i,j],p_value_ERAI_fields[i,j],_ = stats.linregress(OMET_GLORYS2V3_white_series[:,lat_interest['GLORYS2V3'][c]],SLP_ERAI_white_series[168:-24,i,j])
    # figsize works for the size of the map, not the entire figure
    fig2 = plt.figure()
    cube_ERAI = iris.cube.Cube(r_value_ERAI_fields,long_name='Correlation coefficient between SLP and OMET',
                               var_name='r',units='1',dim_coords_and_dims=[(latitude_ERAI_iris, 0), (longitude_ERAI_iris, 1)])
    ...
    cube_ERAI.coord('longitude').coord_system = coord_sys
    # suptitle is the title for the figure
    fig2.suptitle('Regression of SLP Anomaly from ERA-Interim on OMET Anomaly (Atlantic) of GLORYS2V3 across %d N' % (lat_interest_list[c]) ,fontsize = 7,y=0.93)
    ax = plt.axes(projection=ccrs.PlateCarree())
    ax.set_global()
    #ax.set_aspect('auto')
    ax.set_aspect('1.0')
    ...
    # show and save plot
    plt.show()
    fig2.savefig(output_path + os.sep + 'SLP' + os.sep + 'OMET_GLORYS2V3_fields_ERAI' + os.sep + "Regression_OMET_GLORYS2V3_atl_%dN_SLP_ERAI_white_correlation_coef.jpeg" % (lat_interest_list[c]),dpi=300)
    plt.close(fig2)
    ...
    for i in np.arange(len(latitude_ERAI_fields)):
        for j in np.arange(len(longitude_ERAI_fields)):
            # return value: slope, intercept, r_value, p_value, stderr
            slope_ERAI_fields[i,j],_,r_value_ERAI_fields[i,j],p_value_ERAI_fields[i,j],_ = stats.linregress(OMET_SODA3_white_series[:,lat_interest['SODA3'][c]],SLP_ERAI_white_series[12:-12,i,j])
    # figsize works for the size of the map, not the entire figure
    fig3 = plt.figure()
    cube_ERAI = iris.cube.Cube(r_value_ERAI_fields,long_name='Correlation coefficient between SLP and OMET',
                               var_name='r',units='1',dim_coords_and_dims=[(latitude_ERAI_iris, 0), (longitude_ERAI_iris, 1)])
    ...
    cube_ERAI.coord('longitude').coord_system = coord_sys
    # suptitle is the title for the figure
    fig3.suptitle('Regression of SLP Anomaly from ERA-Interim on OMET Anomaly (Atlantic) of SODA3 across %d N' % (lat_interest_list[c]) ,fontsize = 7,y=0.93)
    ax = plt.axes(projection=ccrs.PlateCarree())
    ax.set_global()
    #ax.set_aspect('auto')
    ax.set_aspect('1.0')
    ...
    # show and save plot
    plt.show()
    fig3.savefig(output_path + os.sep + 'SLP' + os.sep + 'OMET_SODA3_fields_ERAI' + os.sep + "Regression_OMET_SODA3_atl_%dN_SLP_ERAI_white_correlation_coef.jpeg" % (lat_interest_list[c]),dpi=300)
    plt.close(fig3)
...
for c in np.arange(len(lat_interest_list)):
    ...
    cube_ERAI.coord('longitude').coord_system = coord_sys
    # suptitle is the title for the figure
    fig4.suptitle('Regression of SST Anomaly from ERA-Interim on OMET Anomaly (Atlantic) of ORAS4 across %d N' % (lat_interest_list[c]) ,fontsize = 7,y=0.93)
    ax = plt.axes(projection=ccrs.PlateCarree())
    ax.set_global()
    #ax.set_aspect('auto')
    ax.set_aspect('1.0')
    ...
    # show and save plot
    plt.show()
    #fig4.savefig(output_path + os.sep + 'SST' + os.sep + 'OMET_ORAS4_fields_ERAI' + os.sep + "Regression_OMET_ORAS4_atl_%dN_SST_ERAI_white_correlation_coef.jpeg" % (lat_interest_list[c]),dpi=300)
    fig4.savefig(output_path + os.sep + 'SST' + os.sep + 'Detrend' + os.sep + 'OMET_ORAS4_fields_ERAI' + os.sep + "Regression_OMET_ORAS4_atl_%dN_SST_ERAI_white_correlation_coef.jpeg" % (lat_interest_list[c]),dpi=300)
    plt.close(fig4)
    ...
    cube_ERAI.coord('longitude').coord_system = coord_sys
    # suptitle is the title for the figure
    fig5.suptitle('Regression of SST Anomaly from ERA-Interim on OMET Anomaly (Atlantic) of GLORYS2V3 across %d N' % (lat_interest_list[c]) ,fontsize = 7,y=0.93)
    ax = plt.axes(projection=ccrs.PlateCarree())
    ax.set_global()
    #ax.set_aspect('auto')
    ax.set_aspect('1.0')
    ...
    # show and save plot
    plt.show()
    #fig5.savefig(output_path + os.sep + 'SST' + os.sep + 'OMET_GLORYS2V3_fields_ERAI' + os.sep + "Regression_OMET_GLORYS2V3_atl_%dN_SST_ERAI_white_correlation_coef.jpeg" % (lat_interest_list[c]),dpi=300)
    fig5.savefig(output_path + os.sep + 'SST' + os.sep + 'Detrend' + os.sep + 'OMET_GLORYS2V3_fields_ERAI' + os.sep + "Regression_OMET_GLORYS2V3_atl_%dN_SST_ERAI_white_correlation_coef.jpeg" % (lat_interest_list[c]),dpi=300)
    plt.close(fig5)
    ...
    cube_ERAI.coord('longitude').coord_system = coord_sys
    # suptitle is the title for the figure
    fig6.suptitle('Regression of SST Anomaly from ERA-Interim on OMET Anomaly (Atlantic) of SODA3 across %d N' % (lat_interest_list[c]) ,fontsize = 7,y=0.93)
    ax = plt.axes(projection=ccrs.PlateCarree())
    ax.set_global()
    #ax.set_aspect('auto')
    ax.set_aspect('1.0')
    ...
    # show and save plot
    plt.show()
    #fig6.savefig(output_path + os.sep + 'SST' + os.sep + 'OMET_SODA3_fields_ERAI' + os.sep + "Regression_OMET_SODA3_atl_%dN_SST_ERAI_white_correlation_coef.jpeg" % (lat_interest_list[c]),dpi=300)
    fig6.savefig(output_path + os.sep + 'SST' + os.sep + 'Detrend' + os.sep + 'OMET_SODA3_fields_ERAI' + os.sep + "Regression_OMET_SODA3_atl_%dN_SST_ERAI_white_correlation_coef.jpeg" % (lat_interest_list[c]),dpi=300)
    plt.close(fig6)
...
for c in np.arange(len(lat_interest_list)):
    ...
    for i in np.arange(len(latitude_ERAI_fields)):
        for j in np.arange(len(longitude_ERAI_fields)):
            # return value: slope, intercept, r_value, p_value, stderr
            slope_ERAI_fields[i,j],_,r_value_ERAI_fields[i,j],p_value_ERAI_fields[i,j],_ = stats.linregress(OMET_ORAS4_white_series[:,lat_interest['ORAS4'][c]],TS_ERAI_white_series[:-24,i,j])
    # figsize works for the size of the map, not the entire figure
    fig7 = plt.figure()
    cube_ERAI = iris.cube.Cube(r_value_ERAI_fields,long_name='Correlation coefficient between TS and OMET',
                               var_name='r',units='1',dim_coords_and_dims=[(latitude_ERAI_iris, 0), (longitude_ERAI_iris, 1)])
    cube_ERAI.coord('latitude').coord_system = coord_sys
    cube_ERAI.coord('longitude').coord_system = coord_sys
    # suptitle is the title for the figure
    fig7.suptitle('Regression of TS Anomaly from ERA-Interim on OMET Anomaly (Atlantic) of ORAS4 across %d N' % (lat_interest_list[c]) ,fontsize = 7,y=0.93)
    ax = plt.axes(projection=ccrs.PlateCarree())
    ax.set_global()
    #ax.set_aspect('auto')
    ax.set_aspect('1.0')
    ...
    # show and save plot
    plt.show()
    fig7.savefig(output_path + os.sep + 'TS' + os.sep + 'OMET_ORAS4_fields_ERAI' + os.sep + "Regression_OMET_ORAS4_atl_%dN_TS_ERAI_white_correlation_coef.jpeg" % (lat_interest_list[c]),dpi=300)
    plt.close(fig7)
    ...
    for i in np.arange(len(latitude_ERAI_fields)):
        for j in np.arange(len(longitude_ERAI_fields)):
            # return value: slope, intercept, r_value, p_value, stderr
            slope_ERAI_fields[i,j],_,r_value_ERAI_fields[i,j],p_value_ERAI_fields[i,j],_ = stats.linregress(OMET_GLORYS2V3_white_series[:,lat_interest['GLORYS2V3'][c]],TS_ERAI_white_series[168:-24,i,j])
    # figsize works for the size of the map, not the entire figure
    fig8 = plt.figure()
    cube_ERAI = iris.cube.Cube(r_value_ERAI_fields,long_name='Correlation coefficient between TS and OMET',
                               var_name='r',units='1',dim_coords_and_dims=[(latitude_ERAI_iris, 0), (longitude_ERAI_iris, 1)])
    ...
    cube_ERAI.coord('longitude').coord_system = coord_sys
    # suptitle is the title for the figure
    fig8.suptitle('Regression of TS Anomaly from ERA-Interim on OMET Anomaly (Atlantic) of GLORYS2V3 across %d N' % (lat_interest_list[c]) ,fontsize = 7,y=0.93)
    ax = plt.axes(projection=ccrs.PlateCarree())
    ax.set_global()
    #ax.set_aspect('auto')
    ax.set_aspect('1.0')
    ...
    # show and save plot
    plt.show()
    fig8.savefig(output_path + os.sep + 'TS' + os.sep + 'OMET_GLORYS2V3_fields_ERAI' + os.sep + "Regression_OMET_GLORYS2V3_atl_%dN_TS_ERAI_white_correlation_coef.jpeg" % (lat_interest_list[c]),dpi=300)
    plt.close(fig8)
    ...
    for i in np.arange(len(latitude_ERAI_fields)):
        for j in np.arange(len(longitude_ERAI_fields)):
            # return value: slope, intercept, r_value, p_value, stderr
            slope_ERAI_fields[i,j],_,r_value_ERAI_fields[i,j],p_value_ERAI_fields[i,j],_ = stats.linregress(OMET_SODA3_white_series[:,lat_interest['SODA3'][c]],TS_ERAI_white_series[12:-12,i,j])
    # figsize works for the size of the map, not the entire figure
    fig9 = plt.figure()
    cube_ERAI = iris.cube.Cube(r_value_ERAI_fields,long_name='Correlation coefficient between TS and OMET',
                               var_name='r',units='1',dim_coords_and_dims=[(latitude_ERAI_iris, 0), (longitude_ERAI_iris, 1)])
    ...
    cube_ERAI.coord('longitude').coord_system = coord_sys
    # suptitle is the title for the figure
    fig9.suptitle('Regression of TS Anomaly from ERA-Interim on OMET Anomaly (Atlantic) of SODA3 across %d N' % (lat_interest_list[c]) ,fontsize = 7,y=0.93)
    ax = plt.axes(projection=ccrs.PlateCarree())
    ax.set_global()
    #ax.set_aspect('auto')
    ax.set_aspect('1.0')
    ...
    # show and save plot
    plt.show()
    fig9.savefig(output_path + os.sep + 'TS' + os.sep + 'OMET_SODA3_fields_ERAI' + os.sep + "Regression_OMET_SODA3_atl_%dN_TS_ERAI_white_correlation_coef.jpeg" % (lat_interest_list[c]),dpi=300)
    plt.close(fig9)
