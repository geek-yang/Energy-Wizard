...
def ocean_heat_content(theta_key):
    ...
    for i in np.arange(level):
        OHC_globe[i,:,:] = constant['rho'] * constant['cp'] * T_vgrid[i,:,:].filled() * e1t * e2t * e3t_0[i] * tmask[i,:,:] -\
                           constant['rho'] * constant['cp'] * T_vgrid[i,:,:].filled() * e1t * e2t * e3t_adjust[i,:,:] * tmask[i,:,:]
        OHC_atlantic[i,:,:] = constant['rho'] * constant['cp'] * T_vgrid[i,:,:].filled() * e1t * e2t * e3t_0[i] * tmask[i,:,:] * tmaskatl -\
                              constant['rho'] * constant['cp'] * T_vgrid[i,:,:].filled() * e1t * e2t * e3t_adjust[i,:,:] * tmask[i,:,:] * tmaskatl
    # take the zonal integral
