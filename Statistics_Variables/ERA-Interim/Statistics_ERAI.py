...
def field_statistics(T_q_key, z_lnsp_key, u_v_key, gz):
    # extract variables
...
def create_netcdf_point(T_vert_mean_pool, u_vert_mean_pool, v_vert_mean_pool, q_vert_mean_pool, gz_vert_mean_pool,\
    ...
    u_vert_wrap_var = data_wrap.createVariable('u_vert_mean',np.float64,('month','latitude','longitude'))
    v_vert_wrap_var = data_wrap.createVariable('v_vert_mean',np.float64,('month','latitude','longitude'))
    q_vert_wrap_var = data_wrap.createVariable('q_vert_mean',np.float64,('month','latitude','longitude'))
    gz_vert_wrap_var = data_wrap.createVariable('gz_vert_mean',np.float64,('month','latitude','longitude'))
    # zonal mean
    T_zonal_wrap_var = data_wrap.createVariable('T_zonal_mean',np.float64,('month','level','latitude'))
    u_zonal_wrap_var = data_wrap.createVariable('u_zonal_mean',np.float64,('month','level','latitude'))
    v_zonal_wrap_var = data_wrap.createVariable('v_zonal_mean',np.float64,('month','level','latitude'))
    q_zonal_wrap_var = data_wrap.createVariable('q_zonal_mean',np.float64,('month','level','latitude'))
    gz_zonal_wrap_var = data_wrap.createVariable('gz_zonal_mean',np.float64,('month','level','latitude'))
    # global attributes
...
if __name__=="__main__":
    ...
    u_vert_mean_pool = np.zeros((Dim_month, Dim_latitude, Dim_longitude),dtype = float)
    v_vert_mean_pool = np.zeros((Dim_month, Dim_latitude, Dim_longitude),dtype = float)
    q_vert_mean_pool = np.zeros((Dim_month, Dim_latitude, Dim_longitude),dtype = float)
    gz_vert_mean_pool = np.zeros((Dim_month, Dim_latitude, Dim_longitude),dtype = float)
    # data pool for zonal mean
    T_zonal_mean_pool = np.zeros((Dim_month, Dim_level_interpolate, Dim_latitude),dtype = float)
    u_zonal_mean_pool = np.zeros((Dim_month, Dim_level_interpolate, Dim_latitude),dtype = float)
    v_zonal_mean_pool = np.zeros((Dim_month, Dim_level_interpolate, Dim_latitude),dtype = float)
    q_zonal_mean_pool = np.zeros((Dim_month, Dim_level_interpolate, Dim_latitude),dtype = float)
    gz_zonal_mean_pool = np.zeros((Dim_month, Dim_level_interpolate, Dim_latitude),dtype = float)
    ...
    for i in period:
        for j in index_month:
            ...
            T_zonal_mean, u_zonal_mean, v_zonal_mean, q_zonal_mean, gz_zonal_mean = field_statistics(T_q_key, z_lnsp_key, u_v_key, gz)
            ...
            u_vert_mean_pool[j-1,:,:] = u_vert_mean
            v_vert_mean_pool[j-1,:,:] = v_vert_mean
            q_vert_mean_pool[j-1,:,:] = q_vert_mean
            gz_vert_mean_pool[j-1,:,:] = gz_vert_mean
            # save the zonal mean terms to the warehouse
            T_zonal_mean_pool[j-1,:,:] = T_zonal_mean
            u_zonal_mean_pool[j-1,:,:] = u_zonal_mean
            v_zonal_mean_pool[j-1,:,:] = v_zonal_mean
            q_zonal_mean_pool[j-1,:,:] = q_zonal_mean
            gz_zonal_mean_pool[j-1,:,:] = gz_zonal_mean
        # save data as netcdf file
        create_netcdf_point(T_vert_mean_pool, u_vert_mean_pool, v_vert_mean_pool, q_vert_mean_pool, gz_vert_mean_pool,\
                            T_zonal_mean_pool, u_zonal_mean_pool, v_zonal_mean_pool, q_zonal_mean_pool, gz_zonal_mean_pool,output_path,i)
