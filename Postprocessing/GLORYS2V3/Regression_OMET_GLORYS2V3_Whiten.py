...
# sea ice cover
ci = dataset_y.variables['ci'][168:432,0:lat_y+1,:]  # from 1979 - 2014
...
time = dataset_y.variables['time'][168:432]
...
plt.plot(index,OMET_white_series[:,lat_interest['GLORYS2V3'][4]],'b--',linewidth = 0.5,label='Anomalies')
plt.plot(index,poly_fit_OMET[:,lat_interest['GLORYS2V3'][4]],'r-',linewidth = 2,label='Polynomial')
plt.plot(index,OMET_white_detrend_series[:,lat_interest['GLORYS2V3'][4]],'g-',linewidth = 1,label='Detrend')
...
for c in np.arange(len(lat_interest_list)):
    ...
    for i in np.arange(lat_y+1):
        for j in np.arange(len(lon)):
            # return value: slope, intercept, r_value, p_value, stderr
            slope[i,j],_,r_value[i,j],p_value_original[i,j],_ = stats.linregress(OMET_white_detrend_series[:,lat_interest['GLORYS2V3'][c]],ci_white_detrend[:,i,j])
    ...
    for i in np.arange(lat_y+1):
        for j in np.arange(len(lon)):
            # return value: slope, intercept, r_value, p_value, stderr
            slope[i,j],_,r_value[i,j],p_value[i,j],_ = stats.linregress(OMET_white_detrend_running_mean[:,lat_interest['GLORYS2V3'][c]],ci_white_detrend_running_mean[:,i,j])
