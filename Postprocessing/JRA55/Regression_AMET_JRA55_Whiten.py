...
mask_SST = dataset_y.variables['mask'][0:lat_y+1,:]
# sea ice cover
ci = dataset_y.variables['SIC'][21:,:,0:lat_y+1,:]
mask_ci = dataset_y.variables['mask'][0:lat_y+1,:]
# longitude
...
for i in np.arange(len(lat)):
    for j in np.arange(len(lon)):
        polynomial = np.polyfit(np.arange(len(time)), ci_white_series[:,i,j], 3)
...
plt.title('Sea Ice Concentration Anomalies in the Arctic and the detrend SIC anomalies (1979-2013)')
plt.legend()
...
plt.show()
fig002.savefig(output_path + os.sep + 'Detrend_poly_JRA55_ice_white.jpg', dpi = 300)
...
cs = m.contourf(XX,YY,a*12*10,color,cmap='coolwarm',extend='both')
# add color bar
...
plt.title('Trend of Sea Level Pressure Anomalies (1979-2013)',fontsize = 9, y=1.05)
plt.show()
fig11.savefig(output_path + os.sep + "Trend_JRA55_SLP.jpeg",dpi=400)
...
# !!!!!take care about the coordinate of contourf(Longitude, Latitude, data(Lat,Lon))
cs = m.contourf(XX,YY,np.ma.masked_where(mask_SST,a*12*10),color,cmap='coolwarm',extend='both')
# add color bar
...
# !!!!!take care about the coordinate of contourf(Longitude, Latitude, data(Lat,Lon))
cs = m.contourf(XX,YY,np.ma.masked_where(mask_ci,a*12*10),color,cmap='coolwarm',extend='both')
# add color bar
...
for c in np.arange(len(lat_interest_list)):
    ...
    for i in np.arange(lat_y+1):
        for j in np.arange(len(lon)):
            ...
            slope[i,j],_,r_value[i,j],p_value_original[i,j],_ = stats.linregress(AMET_white_series[:,lat_interest['JRA55'][c]],SLP_white_series[:,i,j])
    # visualization through basemap
    fig14 = plt.figure()
    # setup north polar stereographic basemap
    # resolution c(crude) l(low) i(intermidiate) h(high) f(full)
    ...
    #fig151.savefig(output_path + os.sep + 'SLP' + os.sep + 'Annual' + os.sep + "Regression_AMET_SLP_JRA55_white_%dN_lowpass_%dm_regression_coef.jpeg" % (lat_interest_list[c],window),dpi=400)
    ...
    for i in np.arange(lat_y+1):
        for j in np.arange(len(lon)):
            # return value: slope, intercept, r_value, p_value, stderr
            slope[i,j],_,r_value[i,j],p_value[i,j],_ = stats.linregress(AMET_white_series[:,lat_interest['JRA55'][c]],SST_white_series[:,i,j])
    # visualization through basemap
    fig16 = plt.figure()
    # setup north polar stereographic basemap
    m = Basemap(projection='npstere',boundinglat=60,round=True,lon_0=0,resolution='l')
    ...
    for i in np.arange(lat_y+1):
        for j in np.arange(len(lon)):
            # return value: slope, intercept, r_value, p_value, stderr
            slope[i,j],_,r_value[i,j],p_value[i,j],_ = stats.linregress(AMET_white_series[:,lat_interest['JRA55'][c]],ci_white_series[:,i,j])
    # visualization through basemap
    fig18 = plt.figure()
    # setup north polar stereographic basemap
    m = Basemap(projection='npstere',boundinglat=60,round=True,lon_0=0,resolution='l')
