...
# specify data path
# AMET
datapath_AMET = 'F:\DataBase\HPC_out\MERRA2\postprocessing'
# target climatological variables
datapath_y = "F:\DataBase\ERA_Interim\Monthly"
# specify output path for figures
output_path = 'C:\Yang\PhD\Computation and Modeling\Blue Action\AMET\MERRA2'
...
AMET = dataset_AMET.variables['E'][:,:,lat_AMET]/1000 # from Tera Watt to Peta Watt
year = dataset_AMET.variables['year'][:]
# extract variables from 20N to 90 N
# sea level pressure
SLP = dataset_y.variables['msl'][12:,0:lat_y+1,:]
# sea surface temperature
SST = dataset_y.variables['sst'][12:,0:lat_y+1,:]
mask_SST = np.ma.getmaskarray(SST[0,:,:])
# sea ice cover
ci = dataset_y.variables['ci'][12:,0:lat_y+1,:]
...
time = dataset_y.variables['time'][12:]
...
# These climitology data comes from MERRA2 surface level
...
AMET_running_mean = np.zeros(len(AMET_series)-window+1)
#AMET_running_sum = np.zeros(len(AMET_series)-window+1)
for i in np.arange(len(AMET_series)-window+1):
    AMET_running_mean[i] = np.mean(AMET_series[i:i+window])
    #AMET_running_sum[i] = np.sum(AMET_series[i:i+window])

# calculate the running mean and sum of AMET after removing the seasonal cycling
AMET_white_running_mean = np.zeros(len(AMET_white_series)-window+1)
#AMET_running_sum = np.zeros(len(AMET_series)-window+1)
for i in np.arange(len(AMET_white_series)-window+1):
    AMET_white_running_mean[i] = np.mean(AMET_white_series[i:i+window])
    #AMET_white_running_sum[i] = np.sum(AMET_white_series[i:i+window])
...
plt.plot(index,AMET_white_series,'b-',label='MERRA2')
plt.title('Atmospheric Meridional Energy Transport Anomaly at 60N (1980-2016)')
#plt.legend()
...
plt.show()
fig1.savefig(output_path + os.sep + 'AMET_anomaly_60N_time_series_1980_2016.jpg', dpi = 500)
...
plt.ylabel("Meridional Energy Transport (PW)")
plt.show()
fig4.savefig(output_path + os.sep + 'AMET_anomaly_season_time_series_1980_2016.jpg', dpi = 500)
...
plt.show()
fig8.savefig(output_path + os.sep + 'AMET_60N_FFT_1980_2016.jpg', dpi = 500)
...
plt.ylabel("Power spectrum density (PW^2/month)")
plt.show()
fig9.savefig(output_path + os.sep + 'AMET_anomaly_60N_FFT_1980_2016.jpg', dpi = 500)
...
plt.ylabel("Power spectrum density (PW^2/month)")
plt.show()
fig10.savefig(output_path + os.sep + 'AMET_anomaly_60N_FFT_running_mean_%d_1980_2016.jpg' % (window), dpi = 500)
...
plt.show()
fig11.savefig(output_path + os.sep + "Trend_MERRA2_SLP.jpeg",dpi=500)
...
plt.show()
fig12.savefig(output_path + os.sep + "Trend_MERRA2_SST.jpeg",dpi=500)
...
plt.show()
fig13.savefig(output_path + os.sep + "Trend_MERRA2_Ice.jpeg",dpi=500)
...
for i in np.arange(lat_y+1):
    for j in np.arange(len(lon)):
        ...
        slope[i,j],_,r_value[i,j],p_value[i,j],_ = stats.linregress(AMET_white_series,SLP_white_selected[:,i,j])
# visualization through basemap
fig14 = plt.figure()
# setup north polar stereographic basemap
# resolution c(crude) l(low) i(intermidiate) h(high) f(full)
...
# !!!!!take care about the coordinate of contourf(Longitude, Latitude, data(Lat,Lon))
cs = m.contourf(XX,YY,r_value,color,cmap='coolwarm') # SLP_white
# add color bar
...
plt.title('Regression of SLP Anomaly on AMET Anomaly across 60 N',fontsize = 9, y=1.05)
plt.show()
fig14.savefig(output_path + os.sep + "Regression_AMET_SLP_MERRA2_white_correlation_coef.jpeg",dpi=500)
...
plt.title('Regression of SLP Anomaly on AMET Anomaly across 60 N',fontsize = 9, y=1.05)
plt.show()
fig15.savefig(output_path + os.sep + "Regression_AMET_SLP_MERRA2_white_regression_coef.jpeg",dpi=500)
...
for i in np.arange(lat_y+1):
    for j in np.arange(len(lon)):
        # return value: slope, intercept, r_value, p_value, stderr
        slope[i,j],_,r_value[i,j],p_value[i,j],_ = stats.linregress(AMET_white_series,SST_white_selected[:,i,j])
# visualization through basemap
fig16 = plt.figure()
# setup north polar stereographic basemap
m = Basemap(projection='npstere',boundinglat=60,round=True,lon_0=0,resolution='l')
...
# get the coordinate on the map (lon,lat) and plot scatter dots
m.scatter(XX[i,j],YY[i,j],2.2,marker='.',color='g',alpha=0.6, edgecolor='none') # alpha bleding factor with map
plt.title('Regression of SST Anomaly on AMET Anomaly across 60 N ',fontsize = 9, y=1.05)
plt.show()
fig16.savefig(output_path + os.sep + "Regression_AMET_SST_MERRA2_white_correlation_coef.jpeg",dpi=500)
...
plt.title('Regression of SST Anomaly on AMET Anomaly across 60 N',fontsize = 9, y=1.05)
plt.show()
fig17.savefig(output_path + os.sep + "Regression_AMET_SST_MERRA2_white_regression_coef.jpeg",dpi=500)
...
for i in np.arange(lat_y+1):
    for j in np.arange(len(lon)):
        # return value: slope, intercept, r_value, p_value, stderr
        slope[i,j],_,r_value[i,j],p_value[i,j],_ = stats.linregress(AMET_white_series,ci_white_selected[:,i,j])
# visualization through basemap
fig18 = plt.figure()
# setup north polar stereographic basemap
m = Basemap(projection='npstere',boundinglat=60,round=True,lon_0=0,resolution='l')
...
# get the coordinate on the map (lon,lat) and plot scatter dots
m.scatter(XX[i,j],YY[i,j],2.2,marker='.',color='g',alpha=0.6, edgecolor='none') # alpha bleding factor with map
plt.title('Regression of Sea Ice Concentration Anomaly on AMET Anomaly across 60 N',fontsize = 9, y=1.05)
plt.show()
fig18.savefig(output_path + os.sep + "Regression_AMET_Ice_MERRA2_white_correlation_coef.jpeg",dpi=500)
...
XX, YY = m(xx, yy)
# define color range for the contourf
color = np.linspace(-0.12,0.12,13)
# !!!!!take care about the coordinate of contourf(Longitude, Latitude, data(Lat,Lon))
cs = m.contourf(XX,YY,slope,color,cmap='coolwarm')
# add color bar
cbar = m.colorbar(cs,location="bottom",size='4%',pad="8%",format='%.2f',ticks=[-0.12,-0.06,0,0.06,0.12])
cbar.ax.tick_params(labelsize=8)
#cbar.set_ticks(np.arange(0,6))
cbar_labels = ['-12%','-6%','0%','6%','12%']
cbar.ax.set_xticklabels(cbar_labels)
...
plt.title('Regression of Sea Ice Concentration Anomaly on AMET Anomaly across 60 N',fontsize = 9, y=1.05)
plt.show()
fig19.savefig(output_path + os.sep + "Regression_AMET_Ice_MERRA2_white_regression_coef.jpeg",dpi=500)
